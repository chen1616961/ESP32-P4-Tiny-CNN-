 ...
 * - recognition_method controls off/mlp/tinycls/yolo26/yolo11 at runtime
 * - network_mode controls sta/softap/apsta at runtime and is stored in NVS
 *
 * 中文结构说明：
 * - 摄像头任务负责 Wake/Standby、采集、编码和发布最新帧，网页命令只入队，不阻塞 HTTP。
 * - 独立 inference_task 负责 YOLO11/YOLO26/COCO 长耗时推理，摄像头和 /stream 不等待模型跑完。
 * - HTTP worker 并发服务多个 /stream 客户端，所有客户端读取同一份最新帧缓存。
 * - 历史任务把抽帧识别结果写入 RAM 环形队列和 TF 卡，并按数量/索引大小自动淘汰旧数据。
 * - 识别方法运行时可切换：off 用来测纯图传，mlp 是轻量 baseline，tinycls 是海面目标 Tiny CNN，YOLO/COCO 保留作对比。
...
#define CONFIG_APP_INFERENCE_INTERVAL_MS 0
#endif
...
#define CONFIG_APP_DEFAULT_RECOGNITION_METHOD 5
#endif
...
static volatile recognition_method_t s_recognition_method = CONFIG_APP_DEFAULT_RECOGNITION_METHOD == 5 ? RECOGNITION_METHOD_TINYCLS :
...
static void top_k_to_json(char *buf, size_t size, const vision_result_t *vision)
{
    ...
    for (uint32_t i = 0; i < count && off < size; i++) {
        const tiny_cls_topk_t *top = &vision->top_k[i];
        n = snprintf(buf + off, size - off,
                     "%s{\"label\":\"%s\",\"class_id\":%" PRIu32 ",\"score\":%" PRIu32 "}",
                     i == 0 ? "" : ",", top->label, top->class_id, top->score);
        if (n < 0) {
            break;
        }
        off += (size_t)n;
...
"<div class=\"actions\"><label class=\"toggle\"><input id=\"visionToggle\" type=\"checkbox\" onchange=\"setVision(this.checked)\"><span>识别 Vision</span></label><select id=\"methodSelect\" onchange=\"setMethod(this.value)\"><option value=\"off\">关闭 Off</option><option value=\"mlp\">MLP</option><option value=\"tinycls\">Tiny CNN Marine</option><option value=\"coco\">COCO YOLO11n 320</option><option value=\"yolo26\">YOLO26 Coke/Sprite</option><option value=\"yolo11\">YOLO11 Coke/Sprite</option></select><select id=\"netSelect\" onchange=\"setNetMode(this.value)\"><option value=\"sta\">STA</option><option value=\"softap\">热点 SoftAP</option><option value=\"apsta\">AP+STA</option></select><button id=\"wakeBtn\" onclick=\"cmd('wake')\">唤醒 Wake</button><button id=\"standbyBtn\" onclick=\"cmd('standby')\">待机 Standby</button><a href=\"/validate\">手机验证</a></div></section>"
 ...
 * - 当前固件加载的模型文件见 main/CMakeLists.txt，Tiny CNN 分类模型直接处理 raw frame。
 * - C++ 桥接层负责 raw frame 预处理、ESP-DL 推理和分类 top-k 后处理；YOLO/COCO stub 只保留旧 API 兼容。
 * - C 主程序只关心统一的 vision_result_t，这样网页、历史记录和 API 不需要知道底层是 MLP、YOLO 还是 Tiny CNN。
 */
...
static bool recognition_method_supports_box_validation(recognition_method_t method)
{
    return method == RECOGNITION_METHOD_YOLO26 ||
           method == RECOGNITION_METHOD_YOLO11 ||
           method == RECOGNITION_METHOD_COCO;
...
static bool recognition_method_supports_validation(recognition_method_t method)
{
    return method == RECOGNITION_METHOD_TINYCLS ||
           recognition_method_supports_box_validation(method);
}
...
static bool is_completed_yolo_result(const vision_result_t *vision, recognition_method_t method)
{
    ...
    return strcmp(vision->model, model) == 0 &&
           ...
           (vision->inference_ms > 0 || vision->object_count > 0 ||
            strcmp(vision->label, "no-object") == 0 ||
...
static void apply_tinycls_result(const tiny_cls_espdl_result_t *cls, uint32_t min_score, vision_result_t *result)
{
    ...
    result->object_w = 0;
    result->object_h = 0;
    result->top_k_count = cls->top_k_count;
    memset(result->top_k, 0, sizeof(result->top_k));
    for (uint32_t i = 0; i < cls->top_k_count && i < TINY_CLS_TOP_K; i++) {
    ...
    if (cls->class_id == TINY_CLS_UNKNOWN) {
        result->unknown_score = cls->score;
        result->object_count = 0;
    }
    result->inference_ms = cls->inference_ms;
}

static void classify_tinycls_frame(const uint8_t *data, uint32_t data_size, uint32_t width, uint32_t height,
                                   uint32_t pixel_format, vision_result_t *result)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "Tiny CNN frame inference failed: %s", esp_err_to_name(err));
...
static esp_err_t classify_validation_tinycls_jpeg(const uint8_t *jpeg, uint32_t jpeg_size,
                                                  uint32_t box_min_score,
                                                  vision_result_t *result,
                                                  uint32_t *source_w,
                                                  uint32_t *source_h)
{
    ...
    *source_h = 0;
    ...
    strlcpy(result->model, TINYCLS_MODEL_NAME, sizeof(result->model));
    strlcpy(result->object, "unknown", sizeof(result->object));
    strlcpy(result->label, "tinycls-validation", sizeof(result->label));
    result->box_min_score = box_min_score;
    result->unknown_score = 100;
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "Tiny CNN validation inference failed: %s", esp_err_to_name(err));
        strlcpy(result->label, "tinycls-error", sizeof(result->label));
        return err;
    }
    ...
    while (true) {
        ...
        if (job.validation) {
            ...
            if (job.method == RECOGNITION_METHOD_TINYCLS) {
                ...
                err = classify_validation_yolo_jpeg(job.jpeg, job.jpeg_size, job.method,
                                                    box_min_score, &vision, &source_w, &source_h);
            }
            ...
            if (ctx) {
                ...
                if (vision.object_count > 0 || vision.detection_count > 0) {
        ...
        if (!stale) {
            ...
            history_maybe_queue(job.jpeg, job.jpeg_size, &job.meta, job.method, "camera",
                                job.method == RECOGNITION_METHOD_TINYCLS ?
...
static esp_err_t camera_capture_once(void)
{
     ...
     * 3. MLP/Tiny CNN 很轻，仍在摄像头任务里同步跑；YOLO/COCO 只把 JPEG 副本投递给 inference_task；
     * 4. 立即更新 latest frame，HTTP 推流线程只复制最新 JPEG，不等待 YOLO 模型返回；
     * 5. YOLO/COCO 结果稍后由 inference_task 写回 latest meta，并按历史记录间隔入队保存。
     */
    int64_t start_us = esp_timer_get_time();
    buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    buf.memory = V4L2_MEMORY_MMAP;
    ...
    if (s_camera.pixel_format == V4L2_PIX_FMT_JPEG) {
        ...
        if (run_recognition) {
            ...
            if (method == RECOGNITION_METHOD_MLP || method == RECOGNITION_METHOD_TINYCLS) {
                update_inference_fps(esp_timer_get_time() / 1000);
            }
        ...
        if (need_yolo) {
            ...
            history_maybe_queue(jpeg_ptr, jpeg_size, &meta, method, "camera",
                                method == RECOGNITION_METHOD_TINYCLS ?
                                    meta.vision.object_count > 0 : meta.vision.detection_count > 0);
        }
        s_frames_total++;
...
static void load_runtime_settings(void)
{
    ...
    if (nvs_get_u32(nvs, "version", &version) != ESP_OK || version != SETTINGS_VERSION) {
         ...
         * - 主识别路线按 Kconfig 恢复，避免旧实验模型继续占用演示入口。
         */
        s_network_mode = CONFIG_APP_DEFAULT_NETWORK_MODE == 2 ? NETWORK_MODE_APSTA :
                         (CONFIG_APP_DEFAULT_NETWORK_MODE == 1 ? NETWORK_MODE_SOFTAP :
                          NETWORK_MODE_STA);
        s_recognition_method = CONFIG_APP_DEFAULT_RECOGNITION_METHOD == 5 ? RECOGNITION_METHOD_TINYCLS :
                               (CONFIG_APP_DEFAULT_RECOGNITION_METHOD == 4 ? RECOGNITION_METHOD_COCO :
                                (CONFIG_APP_DEFAULT_RECOGNITION_METHOD == 3 ? RECOGNITION_METHOD_YOLO11 :
                                 (CONFIG_APP_DEFAULT_RECOGNITION_METHOD == 2 ? RECOGNITION_METHOD_YOLO26 :
                                  (CONFIG_APP_DEFAULT_RECOGNITION_METHOD == 0 ? RECOGNITION_METHOD_OFF :
        ...
        if (s_recognition_method == RECOGNITION_METHOD_YOLO26 && !yolo26_espdl_available()) {
            ...
            s_recognition_method = RECOGNITION_METHOD_MLP;
        } else if (s_recognition_method == RECOGNITION_METHOD_TINYCLS && !tiny_cls_espdl_available()) {
            s_recognition_method = RECOGNITION_METHOD_MLP;
        }
                                            ...
                                            const char *trial)
{
    ...
    if (!recognition_method_supports_box_validation(method) || !validation_selftest_method_available(method)) {
...
static recognition_method_t parse_validation_method(const char *text)
{
    ...
    return recognition_method_supports_validation(s_recognition_method) ? s_recognition_method : RECOGNITION_METHOD_COCO;
}
...
static esp_err_t validation_run_get_handler(httpd_req_t *req)
{
    ...
    if (!recognition_method_supports_validation(method)) {
        return validation_json_error(req, "400 Bad Request", "validation supports tinycls, coco, yolo11, or yolo26");
    }
    ...
    if (recognition_method_supports_box_validation(method)) {
        fill_yolo_pending(&job.meta.vision, method);
    } else {
        strlcpy(job.meta.vision.model, model_name_for_method(method), sizeof(job.meta.vision.model));
        strlcpy(job.meta.vision.object, "unknown", sizeof(job.meta.vision.object));
        strlcpy(job.meta.vision.label, "tinycls-waiting", sizeof(job.meta.vision.label));
        job.meta.vision.unknown_score = 100;
        job.meta.vision.box_min_score = validation_box_min_score;
    }
    job.jpeg = alloc_psram_buffer(jpeg_size);
    if (!job.jpeg) {
        vSemaphoreDelete(ctx->done);
...
static esp_err_t status_get_handler(httpd_req_t *req)
{
    ...
    top_k_to_json(top_k_json, sizeof(top_k_json), &meta.vision);
...
static esp_err_t vision_get_handler(httpd_req_t *req)
{
    ...
    if (strcmp(enabled, "1") == 0 || strcmp(enabled, "true") == 0 || strcmp(enabled, "on") == 0) {
        ...
        if (s_recognition_method == RECOGNITION_METHOD_OFF) {
            s_recognition_method = tiny_cls_espdl_available() ? RECOGNITION_METHOD_TINYCLS :
                                   (coco_espdl_available() ? RECOGNITION_METHOD_COCO : RECOGNITION_METHOD_MLP);
...
static esp_err_t recognition_get_handler(httpd_req_t *req)
{
    ...
    if (strcmp(method_text, "off") == 0) {
        ...
        s_vision_enabled = true;
    } else if (strcmp(method_text, "tinycls") == 0) {
        if (!tiny_cls_espdl_available()) {
            httpd_resp_set_status(req, "503 Service Unavailable");
            return httpd_resp_sendstr(req, "tinycls board backend is unavailable; check models/tiny_cls_96_6cls_s8_p4.espdl");
        }
        s_recognition_method = RECOGNITION_METHOD_TINYCLS;
        s_vision_enabled = true;
    } else if (method_text[0] != '\0') {
        return httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "supported method: off, mlp, tinycls, coco, yolo26, yolo11");
    }
